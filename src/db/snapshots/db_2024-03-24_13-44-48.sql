...
# Database: db
# Generation time: 2024-03-24T13:45:03+01:00
# ************************************************************
...
CREATE TABLE `page_summaries_view`(
   `id` INT(11)   NOT NULL  ,
	`title` CHAR(25)   NOT NULL  COLLATE utf8mb4_general_ci,
	`content` MEDIUMTEXT   NULL  COLLATE utf8mb4_general_ci
);

DROP TABLE IF EXISTS `page_summaries_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`db`@`%` SQL SECURITY DEFINER VIEW `page_summaries_view` AS select `pages`.`id` AS `id`,`pages`.`title` AS `title`,case when octet_length(`pages`.`content`) > 25 then concat(substr(`pages`.`content`,1,25),'...') else `pages`.`content` end AS `content` from `pages`;
...
# Dump completed on 2024-03-24T13:45:03+01:00
